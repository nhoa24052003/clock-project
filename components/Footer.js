    ...
    <div className="bg-[#000] text-white">
      <div className="md:flex md:justify-between md:items-center sm:px-12 px-4 bg-[#342ac8] py-7">
        <h1 className="lg:text-3xl text-3xl md:mb-0 mb-6 lg:leading-normal font-semibold md:w-2/5">
          <span className="text-[#56d879]">Đăng ký</span> với chúng tôi để nhận tin tức{" "}
          <br /> sự kiện và ưu đãi
        </h1>
        <div>
          <input
            type="text"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-6 px-5 py-16 sm:text-center">
        <ul className="px-5 text-center sm:text-start flex sm:block flex-col items-center">
          ...
          />
          <br />
          <p>Phụ kiện và những yếu tố cần thiết để tạo ra những sản phẩm đẹp.</p>
        ...
        <ul className="text-center sm:text-start">
          ...
          {footerProductLinks &&
            footerProductLinks.map((link, index) => (
              <li key={index}>
                <Link className="text-gray-400 hover:text-blue-600 duration-300 text-sm cursor-pointer leading-6">
                  {link.name}
                </Link>
              </li>
            ))}
